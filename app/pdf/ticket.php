<?php
	...
	if($datos_venta->rowCount()==1){
        ...
        $pdf->Ln(0);
        $pdf->Cell(0,9,iconv("UTF-8", "ISO-8859-1","-------------------------------------------------------------------"),0,0,'C');
        $pdf->Ln(3);
    ...
    // Definimos anchos de columnas
        ...
        // Encabezados
        $pdf->Cell($w_cantidad, 10 ,iconv("UTF-8", "ISO-8859-1", "Cant."), 0, 0, 'C');
        $pdf->Cell($w_producto, 10, iconv("UTF-8", "ISO-8859-1", "Producto."), 0, 0, 'C');
        $pdf->Cell($w_autor, 10, iconv("UTF-8", "ISO-8859-1", "Autor."), 0, 0, 'C');
        $pdf->Cell($w_precio, 10, iconv("UTF-8", "ISO-8859-1", "Precio."), 0, 0, 'C');
        $pdf->Cell($w_total, 10, iconv("UTF-8", "ISO-8859-1", "Total."), 0, 0, 'C');

        $pdf->Ln(2);
        $pdf->Cell(72,10, iconv("UTF-8", "ISO-8859-1", "-------------------------------------------------------------------"), 0, 1, 'C');
        $pdf->Ln(3);
...
foreach ($venta_detalle as $detalle) {
    ...
    // Guardamos la altura máxima de esta fila
    $max_height = 5;
    
    // Cantidad
    $pdf->SetXY($x, $y);
    $pdf->Cell($w_cantidad, 4, iconv("UTF-8", "ISO-8859-1", $detalle['venta_detalle_cantidad']), 0, 0, 'C');
    
    // Producto (con salto de línea automático)
    $pdf->SetXY($x + $w_cantidad, $y);
    $pdf->MultiCell($w_producto, 4, iconv("UTF-8", "ISO-8859-1", $detalle['venta_detalle_descripcion']), 0, 'L');
    $producto_height = $pdf->GetY() - $y;
    $max_height = max($max_height, $producto_height);
    
    // Autor (con salto de línea automático)
    $pdf->SetXY($x + $w_cantidad + $w_producto, $y);
    $pdf->MultiCell($w_autor, 4, iconv("UTF-8", "ISO-8859-1", $detalle['autor']), 0, 'L');
    $autor_height = $pdf->GetY() - $y;
    $max_height = max($max_height, $autor_height);
    
    // Precio
    $pdf->SetXY($x + $w_cantidad + $w_producto + $w_autor, $y);
    $pdf->Cell($w_precio, 4, iconv("UTF-8", "ISO-8859-1", MONEDA_SIMBOLO . number_format($detalle['venta_detalle_precio_venta'], MONEDA_DECIMALES, MONEDA_SEPARADOR_DECIMAL, MONEDA_SEPARADOR_MILLAR)), 0, 0, 'R');
    
    // Total
    $pdf->SetXY($x + $w_cantidad + $w_producto + $w_autor + $w_precio, $y);
    $pdf->Cell($w_total, 4, iconv("UTF-8", "ISO-8859-1", MONEDA_SIMBOLO . number_format($detalle['venta_detalle_total'], MONEDA_DECIMALES, MONEDA_SEPARADOR_DECIMAL, MONEDA_SEPARADOR_MILLAR)), 0, 0, 'R');
    
    // Movemos a la siguiente línea usando la altura máxima
    $pdf->SetY($y + $max_height + 4);
}
