<?php
	...
	if($datos_venta->rowCount()==1){
		...
		$pdf->AddPage();
		$pdf->Image(APP_URL.'app/views/img/allBooksC.jpeg',165,12,35,35,'jpeg');
		...
		$pdf->Cell(28,8,iconv("UTF-8", "ISO-8859-1",'Autor'),1,0,'C',true);
		$pdf->Cell(28,8,iconv("UTF-8", "ISO-8859-1",'Cant.'),1,0,'C',true);
		$pdf->Cell(25,8,iconv("UTF-8", "ISO-8859-1",'Precio'),1,0,'C',true);
		$pdf->Cell(26,8,iconv("UTF-8", "ISO-8859-1",'Total'),1,0,'C',true);
		$pdf->Ln(8);
		...
		foreach($venta_detalle as $detalle){
			// Using 'LR' to add both left and right borders
			$pdf->Cell(80,7,iconv("UTF-8", "ISO-8859-1",$ins_venta->limitarCadena($detalle['venta_detalle_descripcion'],80,"...")),'LR',0,'C');
			$pdf->Cell(40,7,iconv("UTF-8", "ISO-8859-1",$ins_venta->limitarCadena($detalle['autor'],60,"...")),'LR',0,'C');
			$pdf->Cell(15,7,iconv("UTF-8", "ISO-8859-1",$detalle['venta_detalle_cantidad']),'LR',0,'C');
			$pdf->Cell(32,7,iconv("UTF-8", "ISO-8859-1",MONEDA_SIMBOLO.number_format($detalle['venta_detalle_precio_venta'],MONEDA_DECIMALES,MONEDA_SEPARADOR_DECIMAL,MONEDA_SEPARADOR_MILLAR)),'LR',0,'C');
			$pdf->Cell(25,7,iconv("UTF-8", "ISO-8859-1",MONEDA_SIMBOLO.number_format($detalle['venta_detalle_total'],MONEDA_DECIMALES,MONEDA_SEPARADOR_DECIMAL,MONEDA_SEPARADOR_MILLAR)),'LR',0,'C');
			$pdf->Ln(7);
		}
		
		// Add bottom border line after the last row
		$pdf->Cell(11,0,'','T'); // Draw a line across the full width
		...
		$pdf->Cell(110,7,iconv("UTF-8", "ISO-8859-1",''),'',0,'C');
