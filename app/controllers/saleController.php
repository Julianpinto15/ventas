<?php
	...
	class saleController extends mainModel{
		...
		public function agregarProductoCarritoControlador() {
			...
			$check_producto = $this->ejecutarConsulta("SELECT p.*, a.nombre AS autor_nombre FROM producto p LEFT JOIN autor a ON p.idAutor = a.idAutor WHERE p.producto_codigo='$codigo'");
            ...
            foreach($_SESSION['datos_producto_venta'] as $venta_detalle){
				...
				$datos_venta_detalle_reg = [
					...
					[
						...
						"campo_valor" => $venta_detalle['autor'] // Asegúrate de que este campo esté presente
					]
				];
